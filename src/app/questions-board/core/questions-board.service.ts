...
export class QuestionsBoardService {
  ...
  initCompareExploitationPlanTasksFromStep(targetExploitationPlanStepId: string, targetItem: Item, versionedExploitationPlanStepId: string): Observable<QuestionsBoardStep[]> {
    const relatedTaskMetadata = Metadata.all(targetItem.metadata, environment.exploitationPlan.exploitationPlanTaskRelationMetadata);
    if (isEmpty(relatedTaskMetadata) || isEmpty(versionedExploitationPlanStepId)) {
      return observableOf([]);
    } else {
