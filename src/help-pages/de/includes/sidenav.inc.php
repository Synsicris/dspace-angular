<nav id="sidebar">
<!-- Platzhalter deutsch / englisch für Auswahl anderer Sprache -->
<ul class="nav1">

	<li><a href="de-synsicrishelp_index.php" id="nav-index"><span>Startseite</span></a></li>

    <li><a href="de-project-about.php" id="nav-about" class="folder"><span>Das Monitoring-Tool</span></a> <!-- unterschiedliche Titel -->
       <ul class="about nav2">
        	<li><a href="de-project-about-objectives.php" id="nav-about-objectives"><span>Hintergrund</span></a></li> <!-- unterschiedliche Titel -->
            <li><a href="de-project-about-difference.php" id="nav-about-difference"><span>Ziele und Nutzen<!-- für Forschende--></span></a></li> <!-- unterschiedliche Titel -->
        </ul>
    </li>

    <li><a href="de-project-content.php" id="nav-content" class="folder"><span>Wichtigste Infos zu Inhalten</span></a>
    	<ul class="content nav2">
        	<li><a href="de-project-content-structure.php" id="nav-content-structure"><span>Inhalte strukturieren</span></a></li>
            <li><a href="de-project-content-entities.php" id="nav-content-entities"><span>Inhalte - erster Überblick</span></a>
            <!-- Dritte Ebene vorerst deaktiviert, bis geklaert ist, ob sie wirklich notwendig ist
            	<ul class="nav3">
        			<li><a href="de-project-content-entities-proj.php" id="nav-content-entities-proj"><span>Verbundprojekt und Teilprojekt</span></a></li>
            		<li><a href="de-project-content-entities-involved.php" id="nav-content-entities-involved"><span>Beteiligte</span></a></li>
                    <li><a href="de-project-content-entities-objectives.php" id="nav-content-entities-objectives"><span>Ziele</span></a></li>
            		<li><a href="de-project-content-entities-events.php" class="id-entities-events"><span>Interaktion</span></a></li>
            		<li><a href="de-project-content-entities-publication.php" id="nav-content-entities-publication"><span>Veröffentlichungen</span></a></li>
                    <li><a href="de-project-content-entities-applic.php" id="nav-content-entities-applic"><span>Anwendung (Möglichkeiten)</span></a></li>
                    <li><a href="de-project-content-entities-impact.php" id="nav-content-entities-impact"><span>Gesellschaftliche Wirkungen</span></a></li>
        		</ul>
            -->
            </li>
            <!-- 7 Punkte aus der dritten Ebene  vorerst in die zweite Ebene verlegt-->
            <li><a href="de-project-content-entities-proj.php" id="nav-content-entities-proj"><span>Verbundprojekt und Teilprojekt</span></a></li>
            <li><a href="de-project-content-entities-involved.php" id="nav-content-entities-involved"><span>Beteiligte</span></a></li>
            <li><a href="de-project-content-entities-objectives.php" id="nav-content-entities-objectives"><span>Ziele</span></a></li>
            <li><a href="de-project-content-entities-events.php" id="nav-content-entities-events"><span>Interaktion</span></a></li>
            <li><a href="de-project-content-entities-publication.php" id="nav-content-entities-publication"><span>Veröffentlichungen</span></a></li>
            <li><a href="de-project-content-entities-applic.php" id="nav-content-entities-applic"><span>Anwendung (Möglichkeiten)</span></a></li>
            <li><a href="de-project-content-entities-impact.php" id="nav-content-entities-impact"><span>Gesellschaftliche Wirkungen</span></a></li>
        </ul>
    </li>

    <li><a href="de-project-ipw.php" id="nav-ipw" class="folder"><span>Impact Pathway</span></a>
    	<ul class="ipw nav2">
        	<li><a href="de-project-ipw-about.php" id="nav-ipw-about"><span>Hintergrund</span></a></li>
            <li><a href="de-project-ipw-help.php" id="nav-ipw-help"><span>Hilfe und Beispiel</span></a></li>
        </ul>
    </li>

    <li><a href="de-project-wp.php" id="nav-wp" class="folder"><span>Arbeitsplan</span></a>
    	<ul class="wp nav2">
        	<li><a href="de-project-wp-help.php" id="nav-wp-help"><span>Hilfe und Beispiel</span></a></li>
        </ul>
    </li>

    <li><a href="de-project-ep.php" id="nav-ep" class="folder"><span>Verwertungsplan</span></a>
    	<ul class="ep nav2">
        	<li><a href="de-project-ep-about.php" id="nav-ep-about"><span>Hintergrund</span></a></li>
            <li><a href="de-project-ep-help.php" id="nav-ep-help"><span>Hilfe und Beispiel</span></a></li>
        </ul>
    </li>

    <li><a href="de-project-use.php" id="nav-use" class="folder"><span>Navigation und Funktionen</span></a> <!-- unterschiedliche Titel: im Tool -->
    	<ul class="use nav2">
        	<li><a href="de-project-use-items.php" id="nav-use-items"><span>Einträge anlegen</span></a></li>
						<li><a href="de-project-use-role.php" id="nav-use-role"><span>Rollen und Rechte verwalten</span></a></li>
						<li><a href="de-project-use-permissions.php" id="nav-use-permissions"><span>Zugangsrechte zum Projekt</span></a></li>
            <li><a href="de-project-use-versioning.php" id="nav-use-versioning"><span>Versionierung</span></a></li>
            <li><a href="de-project-use-export.php" id="nav-use-export"><span>Export und Drucken</span></a></li> <!-- unterschiedliche Titel -->
        </ul>
    </li>
    <li><a href="de-project-step.php" id="nav-step" class="folder"><span>Schritt-für-Schritt-Anleitungen</span></a> <!-- unterschiedliche Titel -->
    	<ul class="step nav2">
        	<li><a href="de-project-step-proposal.php" id="nav-step-proposal"><span>Antrag</span></a></li>
            <li><a href="de-project-step-livetime.php" id="nav-step-livetime"><span>Projektlaufzeit</span></a></li> <!-- unterschiedliche Titel -->
            <li><a href="de-project-step-completion.php" id="nav-step-completion"><span>Projektabschluss</span></a></li>
            <li><a href="de-project-step-update.php" id="nav-step-update"><span>Nach Projektende</span></a></li>
        </ul>
    </li>

    <li><a href="de-project-pages.php" id="nav-pages" class="folder"><span>Hilfe für einzelne Inhalte</span></a>
    	<ul class="pages nav2">
        	<li><a href="de-project-pages-actor.php" id="nav-pages-actor"><span>Akteursgruppen</span></a></li>
            <li><a href="de-project-pages-ethics.php" id="nav-pages-ethics"><span>Ethische Reflexion</span></a></li>
            <li><a href="de-project-pages-impacts.php" id="nav-pages-impacts"><span>Erwartete<!-- gesellschaftliche --> Wirkungen</span></a></li>
            <li><a href="de-project-pages-gender.php" id="nav-pages-gender"><span>Geschlechteraspekte</span></a></li>
						<li><a href="de-project-pages-publication.php" id="nav-pages-publication"><span>Tipps zum Import</span></a></li>
						<li><a href="de-project-pages-solution.php" id="nav-pages-solution"><span>Lösung/Veränderung/Innovation</span></a></li>
        </ul>
    </li>

    <li><a href="de-project-funder.php" id="nav-funder" class="folder"><span>Informationen für den Förderer</span></a>
			<ul class="funder nav2">
				<li><a href="de-project-funder-role.php" id="nav-funder-role"><span>Rollen und Rechte verwalten</span></a></li>
				<li><a href="de-project-funder-comment.php" id="nav-funder-comment"><span>Kommentarfunktion</span></a></li>
			</ul>
		</li>
</ul>

<!-- <div class="arbeitsstand">Stand: 31. Mai 22</div> -->


</nav>
